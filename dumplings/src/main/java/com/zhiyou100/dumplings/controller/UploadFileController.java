...
import com.zhiyou100.dumplings.controller.enums.UploadFileTypeEnum;
import com.zhiyou100.dumplings.util.DateUtil;
import com.zhiyou100.dumplings.util.FileUtil;
import com.zhiyou100.dumplings.util.FtpUtil;
...
import static com.zhiyou100.dumplings.common.Constants.FILE_PRE_URL;
...
public class UploadFileController {
    ...
    @RequestMapping(value = "/toNginx", method = RequestMethod.POST)
    public Result toNginx(@RequestParam("file")MultipartFile multipartFile){
        ...
        if (uploadFileTypeEnum == UploadFileTypeEnum.ERROR_TYPE) {
            ...
            String filePath = DateUtil.getDate();
            String img = IDUtils.genImageName()+"."+type;
            String imageName = nginxUrl+filePath+"/"+img;
            try {
                InputStream inputStream = multipartFile.getInputStream();
                boolean b = FtpUtil.uploadFile(host,port,user,pass,
                        basepath,filePath,img,inputStream);
                if(b){
                    Result result = ResultGenerator.genSuccessResult();
                    result.setData(imageName);
                    return result;
                }
            } catch (IOException e) {
                e.printStackTrace();
